#[cfg(test)]
use crate::domain::{ cells::finite_plane_cell::FinitePlaneCell, ports::ICell };

#[allow(dead_code)]
const GRID_SIZE: i16 = 10;
#[test]
fn test_new_cell() {
  let cell = FinitePlaneCell::new(0, 0, GRID_SIZE).unwrap();
...
fn test_new_cell_bigger_than_grid_size() {
  let cell = FinitePlaneCell::new(GRID_SIZE, GRID_SIZE, GRID_SIZE);
...
#[test]
fn test_get_left() {
  let cell = FinitePlaneCell::new(1, 0, GRID_SIZE).unwrap();
...
fn test_get_right_bigger_than_grid_size() {
  let cell = FinitePlaneCell::new(GRID_SIZE - 1, 0, GRID_SIZE).unwrap();
...
#[test]
fn test_get_top() {
  let cell = FinitePlaneCell::new(0, 1, GRID_SIZE).unwrap();
...
fn test_get_bottom_bigger_than_grid_size() {
  let cell = FinitePlaneCell::new(0, GRID_SIZE - 1, GRID_SIZE).unwrap();
...
fn test_get_upper_left() {
  let cell = FinitePlaneCell::new(1, 1, GRID_SIZE).unwrap();
...
fn test_get_neighborhood_zero_x_index() {
  let cell = FinitePlaneCell::new(0, 2, GRID_SIZE).unwrap();
  let neighborhood = cell.get_neighborhood();
...
fn test_get_neighborhood_zero_y_index() {
  let cell = FinitePlaneCell::new(2, 0, GRID_SIZE).unwrap();
  let neighborhood = cell.get_neighborhood();
...
fn test_get_neighborhood_max_x_index() {
  let cell = FinitePlaneCell::new(GRID_SIZE - 1, 2, GRID_SIZE).unwrap();
  let neighborhood = cell.get_neighborhood();
...
fn test_get_neighborhood_max_y_index() {
  let cell = FinitePlaneCell::new(2, GRID_SIZE - 1, GRID_SIZE).unwrap();
  let neighborhood = cell.get_neighborhood();
